...
describe('AppController (e2e)', () => {
  ...
  it('should create complete flow: producer -> farm -> harvest -> crop', async () => {
    const producerData = {
      name: "TEST_Complete Flow Producer",
      cpfCnpj: cpf.generate()
    };
    ...
    createdProducerId = producerResponse.body.data.id;
    expect(producerResponse.body.data.name).toBe(producerData.name);

    const farmData = {
      producerId: createdProducerId,
      name: "TEST_Complete Flow Farm",
      city: "Araçatuba",
      state: "São Paulo",
      totalArea: 100,
      vegetationArea: 30,
    ...
    const createdHarvestId = harvestResponse.body.data.id;
    expect(harvestResponse.body.data.year).toBe(harvestData.year);

    const cropData = {
      farmId: createdFarmId,
      harvestId: createdHarvestId,
      name: "TEST_Complete Flow Soja",
      useArableArea: 10
    };
    ...
    const createdCropId = cropResponse.body.data.id;
    ...
    expect(createdHarvestId).toBeDefined();
    expect(createdCropId).toBeDefined();
  });
});